...
@property (assign) float mwidth;
@property (assign) float mheight;
...
- (void)layoutSubviews {
    ...
    self.mwidth = indFrame.size.width + 2 * margin;
    self.mheight = indFrame.size.height + 2 * margin;
    ...
    if (nil != self.labelText) {
        ...
        // Update HUD size
        if (self.mwidth < (lWidth + 2 * margin)) {
            self.mwidth = lWidth + 2 * margin;
        }
        self.mheight = self.mheight + lHeight + PADDING;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect boxRect = CGRectMake(roundf((allRect.size.width - self.mwidth) / 2) + self.xOffset,
                                roundf((allRect.size.height - self.mheight) / 2) + self.yOffset, self.mwidth, self.mheight);
	// Corner radius
	float radius = 10.0f;
