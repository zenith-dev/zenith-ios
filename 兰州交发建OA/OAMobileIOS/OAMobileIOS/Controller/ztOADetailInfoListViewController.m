...
- (void)viewDidLoad
{
    ...
    //公文搜索结果列表不显示搜索框
    if ([i_type isEqualToString:@"5"]||[i_type isEqualToString:@"1"]) {
        NSString *placeholderStr = @"";
        //type:1待办公文；2最新通知；3邮件系统；4公告列表;5公文搜索
...
//                [[[[self.searchField.subviews objectAtIndex:0] subviews] objectAtIndex:0] removeFromSuperview];
    ...
    {
        self.table = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, self.view.height-64) style:UITableViewStylePlain];
    }
...
- (void)searchBtnClick
{
    ...
    else
    {
        ...
        {
           searchBarStr = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?><root><strgwbt>%@</strgwbt><strgwz>%@</strgwz><intgwnh>%@</intgwnh><intgwqh>%@</intgwqh><chrgwlb>%@</chrgwlb><strlwdwmc>%@</strlwdwmc><intmjbh>%@</intmjbh><inthjcdbh>%@</inthjcdbh><dtmdjsj1>%@</dtmdjsj1><dtmdjsj2>%@</dtmdjsj2><intzbflbh>%@</intzbflbh></root>",
                              [self UnicodeToISO88591:[NSString stringWithFormat:@"%@",searchField.text?:@"" ]],
                            ...
                            self.searchDic[@"intzbflbh"]];
            NSLog(@"searchdic==%@",searchBarStr);
        }
    }
    [self reflashView];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([i_type isEqualToString:@"2"]) {
        if (indexPath.row<dataListArray.count) {
            ...
            cell.noticeName.text = [NSString stringWithFormat:@"%@",[dataListArray objectAtIndex:indexPath.row]];
