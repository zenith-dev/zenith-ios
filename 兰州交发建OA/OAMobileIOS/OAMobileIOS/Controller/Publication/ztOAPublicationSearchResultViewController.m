...
- (void)viewDidLoad
{
    ...
    [publicSearchBtn setTitle:@"资源共享" forState:UIControlStateNormal];
...
- (void)searchData
{
    NSDictionary *publicSearchDic = [[NSDictionary alloc] initWithObjectsAndKeys:[ztOAGlobalVariable sharedInstance].intrylsh,@"intrylsh",[ztOAGlobalVariable sharedInstance].intdwlsh_child,@"intcsdwlsh",[ztOAGlobalVariable sharedInstance].intdwlsh,@"intdwlsh",[NSString stringWithFormat:@"%d",iRecentPageIndex],@"intCurrentPage",@"10",@"intPageRows",@"16",@"intkwlb",yearSearchStr,@"intkwnh",nil];
    [self showWaitView];
    [ztOAService searchPublicationDirectoryList:publicSearchDic Success:^(id result)
     {
         ...
         if (dicData!=NULL && [[[dicData objectForKey:@"root"] objectForKey:@"result"]intValue]==0) {
             if (((NSMutableArray *)[[dicData objectForKey:@"root"]objectForKey:@"kwlb"]).count>0) {
                 ...
                 iRecentPageIndex++;
                 if ([[[dicData objectForKey:@"root"]objectForKey:@"kwlb"] isKindOfClass:[NSDictionary class]]) {
    ...
    if (infoTable==tableView) {
        if (indexPath.row<dataListArray.count) {
            ...
            if(!cell){
                ...
                UIView *selectView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, cell.contentView.height - 1)];
            ...
            if ([publicSearchStr isEqualToString:@""]) {
                cell.detailInfo.text = [NSString stringWithFormat:@"%@|%@-%@期",[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"chrkwmc"],[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"intkwnh"],[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"intkwqh"]];
            }
            else{
                cell.detailInfo.text = [NSString stringWithFormat:@"%@ %@|%@－%@期",publicSearchStr,[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"chrkwmc"],[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"intkwnh"],[[dataListArray objectAtIndex:indexPath.row] objectForKey:@"intkwqh"]];
            }
            return cell;
        }
        else
