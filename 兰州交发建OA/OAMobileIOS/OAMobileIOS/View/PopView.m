...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.highlightedTextColor=MF_ColorFromRGB(0, 0, 0);
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.delegate && [self.delegate respondsToSelector:@selector(getIndexRow:key:)]) {
        [self.delegate getIndexRow:(int)indexPath.row key:self.key];
    }
    [self hidden];
}
#pragma mark------------显示视图------------------
