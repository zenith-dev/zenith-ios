...
-(void)chooseTagrs:(UIButton*)sender
{
    ...
    else if (lcint==60||lcint==321)
    {
        ...
        //判断是否包含“并行”两字，如果存在则是走多个分流程
        if([[lcdic objectForKey:@"stranbt"] containsString:@"并行"]){
            LBParallelProcessViewController *parallelProcess = [[LBParallelProcessViewController alloc]init];
            parallelProcess.title=[lcdic objectForKey:@"stranbt"];
            parallelProcess.savedic=saveDic;
            parallelProcess.lint=2;
            parallelProcess.bzlcint=lcint;
            [self.nav pushViewController:parallelProcess animated:YES];
        }else{
