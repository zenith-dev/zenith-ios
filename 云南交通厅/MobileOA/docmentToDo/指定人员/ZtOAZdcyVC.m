...
@interface ZtOAZdcyVC()<UITableViewDelegate,UITableViewDataSource,longPressUpDelegate>
{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([allData[indexPath.row] isKindOfClass:[AddUnitInfo class]])
    {
        ...
        if (![self.isdx isEqualToString:@"2"]) {
            ...
            if ([allData[indexPath.row] isKindOfClass:[AddUserInfo class]]) {
                ...
                auser.selectFlg=YES;
                [selectary addObject:allData[indexPath.row]];
...
{
    if ([self.isdx isEqualToString:@"1"]&&selectary.count>1) {
        [self showMessage:@"单选只能选择一个"];
        return;
    }
    self.callback(selectary,self.storeDict);
    [self.navigationController popViewControllerAnimated:YES];
