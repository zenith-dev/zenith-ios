...
@property(nonatomic,strong)UILabel *lwdwnumlb;
@property(nonatomic,strong)UIImageView *lwrightImg;
@end

@implementation HomePageVC
@synthesize titleView,userHeadImgBtn,titleLable,homepageCollection,itemArray,lwdwnumlb,lwrightImg;
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    lwdwnumlb =[[UILabel alloc]initWithFrame:CGRectMake(0, 0, titleView.width-21, 30)];
    lwdwnumlb.hidden=YES;
    [lwdwnumlb setBackgroundColor:RGBACOLOR(0, 0, 0, 0.5)];
    lwdwnumlb.textColor=[UIColor whiteColor];
    lwdwnumlb.font=Font(14);
    lwdwnumlb.userInteractionEnabled=YES;
    [lwdwnumlb bk_whenTapped:^{
        CslwLstVC *cslwlst=[[CslwLstVC alloc]initWithTitle:@"处室来文列表"];
        [self.navigationController pushViewController:cslwlst animated:YES];
    }];
    lwdwnumlb.bottom=titleView.height;
    lwrightImg =[[UIImageView alloc]initWithFrame:CGRectMake(0, lwdwnumlb.top, 16, 16)];
    lwrightImg.right=titleView.width-5;
    [titleView addSubview:lwdwnumlb];
    [titleView addSubview:lwrightImg];
    ...
    itemArray =@[@{@"icon":@"icon_dp",@"bgColor":UIColorFromRGB(0x18b6f7),@"title":@"待办公文"},
                 ...
                 @{@"icon":@"icon_sq",@"bgColor":UIColorFromRGB(0x1ff687b),@"title":@"规划 文档"},
                 ...
                 @{@"icon":@"icon_yj",@"bgColor":UIColorFromRGB(0x83bd07),@"title":@"督办件查询"},
                 @{@"icon":@"icon_telphone",@"bgColor":UIColorFromRGB(0xfaaa21),@"title":@"电话处理单"}];
    ...
    homepageCollection.mj_header=[MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [weakSelf initWithData];
        [weakSelf getGgNum];
        [weakSelf getTznum];
        [weakSelf getCslwNum];
        [weakSelf downloadHeadImage];
    }];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if ([title isEqualToString:@"督办件查询"])
    {
        SdmkListVC *sdmklist=[[SdmkListVC alloc]initWithTitle:title];
        sdmklist.type=@"020";
        [self.navigationController pushViewController:sdmklist animated:YES];
    ...
    else if([title isEqualToString:@"规划 文档"]){
        PlanningDocVC *planndoc=[[PlanningDocVC alloc]initWithTitle:@"规划文档"];
...
#pragma mark----------获取来文登记未读条数--------
-(void)getCslwNum{
    [self networkall:@"document" requestMethod:@"getCslwNum" requestHasParams:@"true" parameter:@{@"intcsdwlsh":@(SingObj.unitInfo.intdwlsh_child)} progresHudText:nil completionBlock:^(id rep) {
        if (rep[@"root"][@"result"]==[NSNull class]||[rep[@"root"][@"result"] intValue]<=0) {
            lwdwnumlb.hidden=YES;
            lwrightImg.hidden=YES;
        }else
        {
            lwdwnumlb.hidden=YES;
            lwrightImg.hidden=YES;
//            lwdwnumlb.hidden=NO;
//            lwrightImg.hidden=NO;
            NSString *  lwdwnum =[NSString stringWithFormat:@"%@",[[rep objectForKey:@"root"] objectForKey:@"result"]];
            lwdwnumlb.text=[NSString stringWithFormat:@"     您有%@条处室来文待登记,请点击查看",lwdwnum];
        }
        [homepageCollection reloadData];
    }];
