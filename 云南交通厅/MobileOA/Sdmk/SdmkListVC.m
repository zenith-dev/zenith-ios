...
- (void)viewDidLoad {
    [super viewDidLoad];
    if ([type isEqualToString:@"010"]||[type isEqualToString:@"011"]||[type isEqualToString:@"012"]||[type isEqualToString:@"013"]||[type isEqualToString:@"020"]) {
...
-(void)getSdmkcxList:(BOOL)isfresh{
    ...
    }else if ([type isEqualToString:@"017"])
    {
        ...
        NSString* params = [NSString stringWithFormat:@"<?xml version=\"1.0\"  encoding=\"UTF-8\"?><root><strgwbt></strgwbt><intrylsh>%@</intrylsh><intcsdwlsh>%@</intcsdwlsh><intdwlsh>%@</intdwlsh><chrgwlb>%@</chrgwlb></root>",@(SingObj.userInfo.intrylsh),@(SingObj.unitInfo.intdwlsh_child),@(SingObj.unitInfo.intdwlsh),type] ;
        NSDictionary *dataDic= [[NSDictionary alloc] initWithObjectsAndKeys: @(numpage),@"intCurrentPage",@"10",@"intPageRows",params,@"queryTermXML",nil];
        [self network:@"document" requestMethod:@"getDcxxList" requestHasParams:@"true" parameter:dataDic progresHudText:isfresh==YES?nil:@"加载中..." completionBlock:^(id rep) {
            if (rep!=nil) {
                if ([rep[@"document"] isKindOfClass:[NSDictionary class]]) {
                    NSDictionary *document = rep[@"document"];
                    SdmkModel *docModel = [SdmkModel new];
                    docModel.chrgwbt = document[@"strwjbt"];
                    docModel.dtmfssj = document[@"dtmdcqssj"];
                    docModel.chrzwr = document[@"strlxzhmc"];
                    docModel.intlxid = document[@"intlxid"];
                    docModel.strblgcbz = document[@"strblgcbz"];
                    docModel.chrzwcs = [NSString stringWithFormat:@"[%@]%@号", document[@"intlxzhnh"], document[@"intlxzhqh"] ] ;
                    
                    if (numpage==1) {
                        listAry =[[NSMutableArray alloc]initWithObjects:docModel, nil];
                        [sdTable.mj_header endRefreshing];
                    }else
                    {
                        [listAry addObject:docModel];
                        [sdTable.mj_footer endRefreshing];
                    }
                    [sdTable reloadData];
                    sdTable.mj_footer.state=MJRefreshStateNoMoreData;
                }else if ([rep[@"document"] isKindOfClass:[NSArray class]]){
                    NSArray *document = rep[@"document"];
                    NSMutableArray *noticeary = [NSMutableArray new];
                    for(id item in document){
                        SdmkModel *docModel = [SdmkModel new];
                        docModel.chrgwbt = item[@"strwjbt"];
                        docModel.dtmfssj = item[@"dtmdcqssj"];
                        docModel.chrzwr = item[@"strlxzhmc"];
                        docModel.strblgcbz = item[@"strblgcbz"];
                        docModel.chrzwcs = [NSString stringWithFormat:@"[%@]%@号", item[@"intlxzhnh"], item[@"intlxzhqh"] ] ;
                        docModel.intlxid = item[@"intlxid"];
                        [noticeary addObject:docModel];
                    }
                    if (numpage==1) {
                        listAry =[[NSMutableArray alloc]initWithArray:noticeary];
                        [sdTable.mj_header endRefreshing];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([listAry[indexPath.row] isKindOfClass:[SdmkModel class]]) {
        ...
        if ([type isEqualToString:@"020"]){
            gwlxxdetail.intlxid = sdmkModel.intlxid;
            gwlxxdetail.type = @"020";
            gwlxxdetail.strblgcbz = sdmkModel.strblgcbz;
        }
