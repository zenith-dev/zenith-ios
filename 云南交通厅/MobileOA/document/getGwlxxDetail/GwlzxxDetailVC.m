...
@property (nonatomic,strong)NSMutableArray *fkfjary;//反馈附件
...
@synthesize intgwlzlsh,slidLabel,gwxqBtn,cllcBtn,tableScrollView,fjary,fkfjary,type,ngrqstr,lzbztb,lbzbary,gwlx,docmentModel,conerightBtn,doctodomodel,sdmkmodel;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)getGwlzxx{
    ...
    }else if([type isEqualToString:@"020"]){//督办
        [self networkall:@"document" requestMethod:@"getDcxxDetails" requestHasParams:@"true" parameter:@{@"intlxid":_intlxid,@"intcsdwlsh":@(SingObj.unitInfo.intdwlsh_child)} progresHudText:@"加载中..." completionBlock:^(id rep) {
            [self getinfoViews:rep[@"root"][@"document"]];
        }];
    }
}
#pragma mark----------------初始化界面-----------------------
-(void)getinfoViews:(NSDictionary *)gwxxDic{
    ...
    //反馈附件
    if ([gwxxDic[@"fkfj"] isKindOfClass:[NSDictionary class]]) {
        [fkfjary addObject:gwxxDic[@"fkfj"]];
    }else if ([gwxxDic[@"fkfj"] isKindOfClass:[NSArray class]])
    {
        [fkfjary addObjectsFromArray:gwxxDic[@"fkfj"]];
    }
    ...
    }else if ([type isEqualToString:@"020"])
    {
        [gwxqBtn setTitle:@"督办信息" forState:UIControlStateNormal];
    }
    ...
    if([_strblgcbz intValue]!=1&&[type isEqualToString:@"020"]){
        tabView.hidden =  YES;
    }
   ...
   //公文信息
    ...
    if ([type isEqualToString:@"015"]) {
        ...
        titleb.text=gwxxDic[@"strwjbt"];//督办标题
    }
    else{
    ...
    {//督办年号期号
        gwzhlb.text=[NSString stringWithFormat:@"%@[%@]%@号",gwxxDic[@"strlxzhmc"],gwxxDic[@"intlxzhnh"],gwxxDic[@"intlxzhqh"]];
    }
    else
    {
    ...
    if ([type isEqualToString:@"020"]) {//督办立项类别
        ...
        hjcdvlb.text=[NSString stringWithFormat:@"%@",gwxxDic[@"strlxlb"]];
        [gwxxsrc addSubview:hjcdvlb];
        hightH=hjcdvlb.bottom+8;
    }
    if ([type isEqualToString:@"020"]) {//督办来文单位
        UILabel *hjcdlb=[[UILabel alloc]initWithFrame:CGRectMake(15, hightH, 80, 18)];
        hjcdlb.font=Font(15);
        hjcdlb.textColor=[UIColor grayColor];
        hjcdlb.text=@"来文单位：";
        ...
        UILabel *hjcdvlb=[[UILabel alloc]initWithFrame:CGRectMake(hjcdlb.right+3, hjcdlb.top, gwzhlb.width-(hjcdlb.right+8), hjcdlb.height)];
        hjcdvlb.font=Font(15);
        hjcdvlb.text=[NSString stringWithFormat:@"%@",gwxxDic[@"strlwdwmc"]];
        [gwxxsrc addSubview:hjcdvlb];
        hightH=hjcdvlb.bottom+8;
    }
    if ([type isEqualToString:@"020"]) {//督办拟办意见
        UILabel *hjcdlb=[[UILabel alloc]initWithFrame:CGRectMake(15, hightH, 80, 18)];
        hjcdlb.font=Font(15);
        hjcdlb.textColor=[UIColor grayColor];
        hjcdlb.text=@"拟办意见：";
        ...
        UILabel *hjcdvlb=[[UILabel alloc]initWithFrame:CGRectMake(hjcdlb.right+3, hjcdlb.top, gwzhlb.width-(hjcdlb.right+8), hjcdlb.height)];
        hjcdvlb.font=Font(15);
        hjcdvlb.text=[NSString stringWithFormat:@"%@",gwxxDic[@"strnbyj"]];
        [gwxxsrc addSubview:hjcdvlb];
        hightH=hjcdvlb.bottom+8;
    }
    if ([type isEqualToString:@"020"]) {//督办办理单位
        UILabel *hjcdlb=[[UILabel alloc]initWithFrame:CGRectMake(15, hightH, 80, 18)];
        hjcdlb.font=Font(15);
        hjcdlb.textColor=[UIColor grayColor];
        hjcdlb.text=@"办理单位：";
        ...
        UILabel *hjcdvlb=[[UILabel alloc]initWithFrame:CGRectMake(hjcdlb.right+3, hjcdlb.top, gwzhlb.width-(hjcdlb.right+8), hjcdlb.height)];
        hjcdvlb.font=Font(15);
        hjcdvlb.text=[NSString stringWithFormat:@"%@",gwxxDic[@"strbldw"]];
        [gwxxsrc addSubview:hjcdvlb];
        hightH=hjcdvlb.bottom+8;
    }
    //缓急程度
    if ([type isEqualToString:@"011"]||[type isEqualToString:@"012"]||[type isEqualToString:@"014"]||[type isEqualToString:@"016"]) {
        ...
        UILabel *hjcdvlb=[[UILabel alloc]initWithFrame:CGRectMake(hjcdlb.right+3, hjcdlb.top, gwzhlb.width-(hjcdlb.right+8), hjcdlb.height)];
        hjcdvlb.font=Font(15);
        hjcdvlb.text=[NSString stringWithFormat:@"%@",gwxxDic[@"strbzhjcd"]];
        [gwxxsrc addSubview:hjcdvlb];
        hightH=hjcdvlb.bottom+8;
    }
    //成文日期
    ...
    //附件
    DealFjVC *dealfjvc = nil;
    if (fjary.count!=0) {
        dealfjvc=[[DealFjVC alloc]initWithFrame:CGRectMake(0, onelb.bottom, kScreenWidth, 30+fjary.count*44) fjAry:fjary type1:[type isEqualToString:@"015"]?4:3 controller:self];
    ...
    DealFjVC *fkfjvc = nil;
    if (fkfjary.count!=0) {
        fkfjvc=[[DealFjVC alloc]initWithFrame:CGRectMake(0, dealfjvc==nil?onelb.bottom: dealfjvc.bottom, kScreenWidth, 30+fkfjary.count*44) fjAry:fkfjary type1:666 controller:self];
        [gwxxsrc addSubview:fkfjvc];
        [gwxxsrc setContentSize:CGSizeMake(gwxxsrc.width, fkfjvc.bottom+10)];
    }
    //督办领导意见
    CGFloat y = 0;
    if(fkfjvc==nil&&dealfjvc==nil){
        y = onelb.bottom;
    }else if(fkfjvc==nil&&dealfjvc!=nil){
        y = dealfjvc.bottom;
    }else{
        y = fkfjvc.bottom;
    }
    UIView *ldLayout = [[UIView alloc]initWithFrame:CGRectMake(0,y, kScreenWidth, 0)];
    UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 10, 80, 18)];
    titleLabel.text = @"领导意见";
    titleLabel.font = Font(16);
    [ldLayout addSubview:titleLabel];
    
    MyLinearLayout *contentLayout = [MyLinearLayout linearLayoutWithOrientation:MyOrientation_Vert];
    contentLayout.padding = UIEdgeInsetsMake(10, 10, 10, 10);
    contentLayout.myHorzMargin = 0;
    contentLayout.heightSize.lBound(ldLayout.heightSize, 10, 1);
    
    [ldLayout addSubview:contentLayout];
    [gwxxsrc addSubview:ldLayout];
    [gwxxsrc setContentSize:CGSizeMake(gwxxsrc.width, ldLayout.bottom+10)];
    if ([gwxxDic[@"ldps"] isKindOfClass:[NSDictionary class]]){
        UILabel *yjnrLabel = [UILabel new];
        yjnrLabel.text = [NSString stringWithFormat:@"%@",gwxxDic[@"ldps"][@"strpsyj"]];
        yjnrLabel.font = Font(15);
        yjnrLabel.textColor = [UIColor blueColor];
        [yjnrLabel sizeToFit];
        yjnrLabel.myLeading = 15;
        yjnrLabel.myTop = 25;
        UILabel *sjLabel = [UILabel new];
        sjLabel.text = gwxxDic[@"ldps"][@"dtmpsrq"];
        sjLabel.font = Font(15);
        [sjLabel sizeToFit];
        sjLabel.myRight = 15;
        sjLabel.myTop = 15;
        [contentLayout addSubview:yjnrLabel];
        [contentLayout addSubview:sjLabel];
    }else if([gwxxDic[@"ldps"] isKindOfClass:[NSArray class]]){
        NSArray *ldps = gwxxDic[@"ldps"];
        for (id item in ldps) {
            UILabel *yjnrLabel = [UILabel new];
            yjnrLabel.text = [NSString stringWithFormat:@"%@",item[@"strpsyj"]];
            yjnrLabel.font = Font(15);
            yjnrLabel.textColor = [UIColor blueColor];
            [yjnrLabel sizeToFit];
            yjnrLabel.myLeading = 15;
            yjnrLabel.myTop = 25;
            UILabel *sjLabel = [UILabel new];
            sjLabel.text = [Tools sjFromStr1:[NSString stringWithFormat:@"%@",item[@"dtmpsrq"]]];
            sjLabel.font = Font(15);
            [sjLabel sizeToFit];
            sjLabel.myRight = 15;
            sjLabel.myTop = 15;
