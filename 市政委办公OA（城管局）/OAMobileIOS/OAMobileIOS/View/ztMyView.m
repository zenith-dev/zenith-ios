...
//线条宽度的数组
static NSMutableArray *lineWidthArray;
//删除线条时删除的线条宽度储存的数组
...
//保存擦除前数据
static int colorCountHistory;
static int widthCountHistory;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        colors=[[NSMutableArray alloc]initWithObjects:[UIColor blackColor],[UIColor whiteColor],[UIColor yellowColor],[UIColor blueColor],[UIColor redColor],[UIColor grayColor],[UIColor purpleColor],[UIColor brownColor],[UIColor cyanColor],[UIColor darkGrayColor],[UIColor greenColor],[UIColor lightGrayColor],[UIColor magentaColor],[UIColor orangeColor],[UIColor colorWithRed:0.400 green:0.400 blue:1.000 alpha:1.000], nil];
        lineWidthArray = [[NSMutableArray alloc] init];
        for (int i = 0;i<50 ;i++) {
            [lineWidthArray addObject:[NSString stringWithFormat:@"%d", (i+1)]];
        }
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([lineArray count]>0) {
        for (int i=0; i<[lineArray count]; i++) {
            ...
            if ([array count]>0)
            {
                ...
                float width=[[lineWidthArray objectAtIndex:widthc] floatValue];
    ...
    if ([pointArray count]>0)
    {
        ...
        float width=[[lineWidthArray objectAtIndex:widthCount] floatValue];
        CGContextSetStrokeColorWithColor(context,[lineColor CGColor]);
        //---------------------------------------------------------------------------
        CGContextSetLineWidth(context, width);
        CGContextStrokePath(context);
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MyViewTouch" object:nil];
}
//手指移动时候发出
...
-(void)clear{
    ...
    colorCountHistory = colorCount;
    colorCount=0;
    [colorArray removeAllObjects];
    [lineArray removeAllObjects];
    [pointArray removeAllObjects];
    [deleWidthArray removeAllObjects];
    widthCountHistory = widthCount;
    widthCount=0;
    [WidthArray removeAllObjects];
    [self setNeedsDisplay];
    //清屏以后可以重绘
    colorCount = colorCountHistory;
    widthCount = widthCountHistory;
    //清屏以后可以重绘，有些问题
    //    deleColorArray=[[NSMutableArray alloc]init];
    //    deleArray=[[NSMutableArray alloc]init];
