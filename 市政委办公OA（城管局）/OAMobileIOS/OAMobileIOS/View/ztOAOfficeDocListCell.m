...
-(void)setModedic:(NSDictionary *)modedic
{
    ...
    self.docName.frame = CGRectMake(self.iconImg.right+5, 2, self.width-self.iconImg.right-5, 20);
    ...
    //发送时间
    self.docSendTime.frame = CGRectMake(self.width-120-10+40, YH(self.docName)+5, 80, 15);
    ...
    self.docSendTime.frame=CGRectMake(self.width-docFontSize.width-5, Y(self.docSendTime), docFontSize.width, H(self.docSendTime));
    ...
    self.line.frame=CGRectMake(0, YH(self.docSendTime)+5, self.width, 0.5);
    self.frame=CGRectMake(X(self), Y(self), W(self), YH(self.line));
}
