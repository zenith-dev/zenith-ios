...
@interface ztOAOfficialDocSearchViewController ()
...
@synthesize toolBar,timeToolBar;
- (id)init
{
    self = [super init];
    if (self) {
...
- (void)viewDidLoad
{
    ...
    self.infoView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 20, self.view.width, 40*10)];
    infoView.backgroundColor = [UIColor clearColor];
    infoView.contentSize = CGSizeMake(self.view.width, 40*9);
    infoView.backgroundColor = [UIColor clearColor];
...
- (void)toSearch
{
    ...
    if (self.i_type==0) {
        NSDictionary *searchdic=@{@"chrgwbt":[self UnicodeToISO88591:[NSString stringWithFormat:@"%@",titleName.text?:@"" ]],
                                  ...
                                  @"chrgwlb":type};
        NSString *dic = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?><root><strgwbt>%@</strgwbt><strgwz>%@</strgwz><intgwqh>%@</intgwqh><chrgwlb>%@</chrgwlb><strlwdwmc>%@</strlwdwmc><intmjbh>%@</intmjbh><inthjcdbh>%@</inthjcdbh><dtmdjsj1>%@</dtmdjsj1><dtmdjsj2>%@</dtmdjsj2><intgwnh>%@</intgwnh></root>",
                         ...
                         @"",
                         startTimeField.text?:@"" ,
                         stopTimeField.text?:@"",yearNum.text?:@""];
        NSLog(@"searchdic==%@",dic);
        //公文查询
    ...
    }else
    {
        ...
        NSLog(@"searchdic==%@",dic);
        //公文查询
        ztOADetailInfoListViewController *listVC=[[ztOADetailInfoListViewController alloc] initWithType:@"6" withTitle:[type isEqualToString:@"001"]?@"收文列表":@"发文列表" queryTerm:dic];
        listVC.searchDic=searchdic;
        [self.navigationController pushViewController:listVC animated:YES];
    }
