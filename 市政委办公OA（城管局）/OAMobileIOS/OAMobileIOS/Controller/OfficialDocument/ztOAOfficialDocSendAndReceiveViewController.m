...
- (void)loadDocInfoData
{
    ...
    if (isSearchFlag==YES) {
        //获取公文基本信息及流转信息(查询功能)
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:[ztOAGlobalVariable sharedInstance].intrylsh,@"intrylsh",i_Intgwlzlsh,@"intgwlzlsh", nil];
        [ztOAService getOfficeDocDetailInfo:dic Success:^(id result){
            [self closeWaitView];
            NSDictionary *dicData = [result objectFromJSONData];
            NSLog(@"%@",[dicData JSONString]);
            if ([[dicData objectForKey:@"gw"] objectForKey:@"result"] !=NULL && [[[dicData objectForKey:@"gw"] objectForKey:@"result"] intValue]==0) {
                ...
                companyNameStr =dicData[@"gw"][@"gwgs"][@"strcbrmc"];
                NSMutableArray *xxArray = [[[dicData objectForKey:@"gw"] objectForKey:@"gwgs"] objectForKey:@"dbxx"] ;
                
                if ([xxArray isKindOfClass:[NSDictionary class]]) {
                    [dbxxArray addObject:xxArray];
                }else
                {
                    dbxxArray =xxArray;
                }
...
- (void)doSave
{
    ...
    if ([nextStepDic[@"intact"] intValue]==112) {
        ZtOAPopView *ztoapop=[[ZtOAPopView alloc]initWithFrame:[UIScreen mainScreen].bounds];
        ztoapop.callback=^(NSDictionary*dic){
            NSString*intlcbh=detailInfoDic[@"gw"][@"gwgs"][@"intlcbh"];
            NSString*intgwlsh=[baseInfoDic objectForKey:@"intgwlsh"];
            NSString*intgwlzlsh=baseInfoDic[@"intgwlzlsh"];
            NSString*intbzjllsh=[baseInfoDic objectForKey:@"intbzjllsh"];
            NSString *strdbryxm=[self UnicodeToISO88591:[ztOAGlobalVariable sharedInstance].username];
            NSString *intrylsh=[ztOAGlobalVariable sharedInstance].intrylsh;
            NSString *strdbzt=[self UnicodeToISO88591:dic[@"xx"]];
            NSString *dtmblsx=dic[@"time"];
            NSString *intact=nextStepDic[@"intact"];
            NSString * xml= [NSString stringWithFormat:@"<intlcbh>%@</intlcbh><intgwlsh>%@</intgwlsh><intgwlzlsh>%@</intgwlzlsh><intbzjllsh>%@</intbzjllsh><strdbryxm>%@</strdbryxm><intrylsh>%@</intrylsh><strdbzt>%@</strdbzt><dtmblsx>%@</dtmblsx><intact>%@</intact>",intlcbh,intgwlsh,intgwlzlsh,intbzjllsh,strdbryxm,intrylsh,strdbzt,dtmblsx,intact];
            NSString *str1 = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?><root><lccz><czmc>%@</czmc>%@<dx>%@</dx><mrcs>%@</mrcs></lccz></root>",
                ...
                NSDictionary *lzDic= [[NSDictionary alloc] initWithObjectsAndKeys:[ztOAGlobalVariable sharedInstance].intrylsh,@"intrylsh",[ztOAGlobalVariable sharedInstance].intdwlsh_child,@"intcsdwlsh",str1,@"queryTermXML",nil];
    ...
    else
    {
        dic = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"UTF-8\"?><root><clyj><yjlx>%@</yjlx><yjmc>%@</yjmc><yjnr>%@</yjnr><mrcs>%@</mrcs></clyj><lccz><czmc>%@</czmc>%@<dx>%@</dx><mrcs>%@</mrcs></lccz></root>",
               ...
               [self UnicodeToISO88591:[opinionSelectedDic objectForKey:@"mrcs"]],
               [self UnicodeToISO88591:[NSString stringWithFormat:@"%@",(nextStepDic==nil?@"":[nextStepDic objectForKey:@"stryjmc"])?:@""]],
               [responsibleManDic objectForKey:@"xml"],
               [self UnicodeToISO88591:@"否"],
               [self UnicodeToISO88591:[nextStepDic objectForKey:@"mrcs"]]];
    }
    ...
    [ztOAService saveOfficeDocFlowInfo:lzDic Success:^(id result){
        ...
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:[resultInfo JSONString] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
        }
        ...
        [self closeWaitView];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"提交超时！" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
    }];
}
#pragma mark-返回
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (processInfoList==tableView)
    {
        ...
        if (indexPath.row<processInfoArray.count)
        {
            ...
            if ([[processInfoArray objectAtIndex:indexPath.row] objectForKey:@"dtmbjsj"]!=NULL && ![[[processInfoArray objectAtIndex:indexPath.row] objectForKey:@"dtmbjsj"] isEqualToString:@""]) {
                cell.sendTimeLabel.text =[ztOASmartTime timeFromStr:[NSString stringWithFormat:@"%@",[[processInfoArray objectAtIndex:indexPath.row]objectForKey:@"dtmbjsj"]]];
            }
            //处理意见
            ...
            [cell setSize:CGSizeMake(cell.width, 100)];
            if(expectedLabelSizeContext.height>15)
            {
                ...
                if ([(NSString *)[isExpandArray objectAtIndex:indexPath.row] isEqualToString:@"2"]) {
                    //收起
                    [cell setSize:CGSizeMake(cell.width, 70)];
                    [cell.blueLineImg setSize:CGSizeMake(1, 70)];
                    [cell.breakLine setOrigin:CGPointMake(20, 70-1)];
                    [cell.opinionLabel setFrame:CGRectMake(20+10, cell.dealInfoLabel.bottom+10, cell.width-40-10, 20)];
                    [cell.openOrCloseBtn setOrigin:CGPointMake(20, cell.opinionLabel.bottom)];
                }
                else
                {//展开
                    ...
                    [cell setSize:CGSizeMake(cell.width, 70+expectedLabelSizeContext.height-15)];
                    [cell.blueLineImg setSize:CGSizeMake(1, 70+expectedLabelSizeContext.height-15)];
                    [cell.breakLine setOrigin:CGPointMake(20, 70+expectedLabelSizeContext.height-15-1)];
                    [cell.openOrCloseBtn setOrigin:CGPointMake(20, cell.opinionLabel.bottom)];
                }
            }
            else
            {
                ...
                [cell setSize:CGSizeMake(cell.width, 50)];
                [cell.blueLineImg setSize:CGSizeMake(1, 50)];
                [cell.breakLine setOrigin:CGPointMake(20, 50-1)];
            }
    ...
    else if (docInfoView == tableView)
    {
        ...
        if (indexPath.row==0)
        {
            ...
            if ([type isEqualToString:@"1"]) {
                ...
                companyLab= [NSString stringWithFormat:@"%@%@",labStr,companyNameStr];//部门名称  拟稿人
                [array addObject:zihaoLable];
                [array addObject:companyLab];
                [array addObject:timeLab];
            }else  {
                //收文
            ...
            [array addObject:stateLab];
            ztOADocDealInfoTitleView *titleView = [[ztOADocDealInfoTitleView alloc] initWithFrame:CGRectMake(0, 0, tableView.width, titleHeight+20*3+50) withHeight:titleHeight withTitleStr:titleLable withWarnStr:warnString withWarnImage:iconImgNameStr infoArray:array];
            if (isSearchFlag==YES) {
                //查询不显示
                [titleView.warnLable setHidden:YES];
                [titleView.warnLedImg setHidden:YES];
            }
             cell.height=titleView.bottom;
            if (dbxxArray.count>0) {
                UILabel *dbxx=[[UILabel alloc]initWithFrame:CGRectMake(10, titleView.bottom+10, kScreenWidth, 21)];
                dbxx.font=Font(14);
                dbxx.text=@"督办信息:";
                [cell addSubview:dbxx];
                float dbhight=dbxx.bottom;
                for (int i=0; i<dbxxArray.count; i++) {
                    NSDictionary *dbdic=dbxxArray[i];
              ...
              UIImageView *lineView = [[UIImageView alloc] initWithFrame:CGRectMake(5, dbhight, kScreenWidth-10, 1)];
                lineView.backgroundColor = MF_ColorFromRGB(234, 234, 234);
                [cell addSubview:lineView];
                cell.height=lineView.bottom;
            }
...
- (void)collectDocAction
{
    ...
    [alert addButtonWithTitle:@"确定" handler:^(void){
        if (conerightBtn.selected==NO) {
            NSDictionary *informDic =@{@"intgwlsh":[NSString stringWithFormat:@"%@",baseInfoDic[@"intgwlsh"]],@"intgwlzlsh":baseInfoDic[@"intgwlzlsh"],@"intrylsh":[ztOAGlobalVariable sharedInstance].intrylsh};
            [self showWaitView];
            [ztOAService setGwgrscfj:informDic Success:^(id result)
             {
                 [self closeWaitView];
                 NSDictionary *dic = [result objectFromJSONData];
        ...
        }else
        {
           NSDictionary *informDic =@{@"intgwlsh":[NSString stringWithFormat:@"%@",baseInfoDic[@"intgwlsh"]],@"intgwlzlsh":baseInfoDic[@"intgwlzlsh"],@"intrylsh":[ztOAGlobalVariable sharedInstance].intrylsh};
            [self showWaitView];
            [ztOAService delGwgrscfj:informDic Success:^(id result)
             {
                 [self closeWaitView];
                 NSDictionary *dic = [result objectFromJSONData];
                 if ([[[dic objectForKey:@"root"] objectForKey:@"result"] intValue]==0 ) {
...
- (void)searchLocalCollect
{
    ...
    if ([[detailInfoDic[@"gw"][@"gwgs"] objectForKey:@"scts"] intValue]!=0) {
