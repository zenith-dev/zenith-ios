...
- (void)switchValueChange:(id)sender
{
    ...
    if (isButtonOn==YES) {
        //开始用户名密码认证
        AuthHelper *helper=[ztOAGlobalVariable sharedInstance].helper;
        [helper relogin];
    }
    else
    {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==1)
    {
        if (indexPath.row==1) {
            ...
            NSLog(@"文件二维码");
            ztOAQRPictureViewController *qrVC = [[ztOAQRPictureViewController alloc] init];
            qrVC.title=@"文件二维码";
    ...
    else if(indexPath.section==2)
    {
        ...
        //退出
        [[ztOAGlobalVariable sharedInstance].helper logoutVpn];
