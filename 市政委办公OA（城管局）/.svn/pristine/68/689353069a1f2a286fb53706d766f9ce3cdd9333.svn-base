...
@property (assign) float hwidth;
@property (assign) float hheight;
...
//        self.indicator = [MBSpinningCircle circleWithSize:NSSpinningCircleSizeLarge color:[UIColor colorWithRed:50.0/255.0 green:155.0/255.0 blue:255.0/255.0 alpha:0.5]];
        CGRect circleRect = self.indicator.frame;
        circleRect.origin = CGPointMake(self.bounds.size.width/2.0 - circleRect.size.width/2.0, -5);
        self.indicator.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin;
        self.indicator.frame = circleRect;
//        ((MBSpinningCircle *)self.indicator).circleSize = NSSpinningCircleSizeLarge;
//        ((MBSpinningCircle *)self.indicator).hasGlow = YES;
//        ((MBSpinningCircle *)self.indicator).isAnimating = YES;
//        ((MBSpinningCircle *)self.indicator).speed = 0.55;
	}
...
- (void)layoutSubviews {
    ...
    self.hwidth = indFrame.size.width + 2 * margin;
    self.hheight = indFrame.size.height + 2 * margin;
    ...
    if (nil != self.labelText) {
        ...
        // Update HUD size
        if (self.hwidth < (lWidth + 2 * margin)) {
            self.hwidth = lWidth + 2 * margin;
        }
        self.hheight = self.hheight + lHeight + PADDING;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect boxRect = CGRectMake(roundf((allRect.size.width - self.hwidth) / 2) + self.xOffset,
                                roundf((allRect.size.height - self.hheight) / 2) + self.yOffset, self.hwidth, self.hheight);
	// Corner radius
	float radius = 10.0f;
