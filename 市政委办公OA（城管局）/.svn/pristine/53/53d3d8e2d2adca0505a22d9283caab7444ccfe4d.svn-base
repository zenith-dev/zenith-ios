...
static CGFloat const kDefaultContentScrollViewMaxHeight = 200.;
...
+ (void)initialize
{
    ...
    dispatch_once(&onceToken, ^ {
        ...
        appearance.titleFont = [UIFont boldSystemFontOfSize:18];
...
- (void)updateContentScrollView
{
    ...
    if (_contentView) {
        ...
        if (CGRectGetWidth(_contentView.frame) < self.containerWidth) {
            ...
            [_contentView setHeight:frame.size.height + self.scrollViewPadding];
        }
        ...
        _contentScrollView.frame = CGRectMake( 0, y, self.containerWidth, [self heightForContentScrollView] + self.scrollViewPadding);
