...
- (void)viewDidLoad {
    ...
    homePageList=[[NSMutableArray alloc]initWithObjects:@"通知",@"待办事项",@"公文查询",@"邮件",@"政务信息",@"党风廉政统计表",nil];
    [self searchVersion];
    [self initview];
...
-(UIView*)getviews :(NSString*)imgurl title:(NSString*)title nums:(NSString*)nums tg:(NSInteger)tag sel:(SEL)sel
{
    ...
    if([imgurl isEqualToString:@"党风廉政统计表"]){
        [logoview setImage:PNGIMAGE(@"更多")];
    }else{
...
-(void)turntoSEL:(UIButton*)sender
{
    ...
    }else if (sender.tag==1006)
    {
        ...
        if ([tjbgn containsString:@"统计表查询"]) {
            LBStatisticsViewController *statisticsview=[[LBStatisticsViewController alloc]init];
            statisticsview.title=titlestr;
            [self.navigationController pushViewController:statisticsview animated:YES];

        }else{
            [Tools showMsgBox:@"您没有权限查看"];
        }
