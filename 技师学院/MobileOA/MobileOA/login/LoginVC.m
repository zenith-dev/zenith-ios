...
#pragma mark-----------------------请求升级接口-----------------
-(void)versionUp{
    NSDictionary* dict = [NSDictionary dictionaryWithContentsOfURL:[NSURL URLWithString:@"https://raw.githubusercontent.com/chendongandroid/iosapp/master/manifest.plist"]];
    if (dict) {
        ...
        if (![newVersion isEqualToString:myVersion]) {
            UIAlertView *alertv=[UIAlertView bk_showAlertViewWithTitle:@"温馨提示" message:@"已有新版本请升级" cancelButtonTitle:@"取消" otherButtonTitles:@[@"确定"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
                                    if (buttonIndex==1) {
       [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"itms-services://?action=download-manifest&url=https://raw.githubusercontent.com/chendongandroid/iosapp/master/manifest.plist"]];
                                        }
                                    }];
                                    [alertv show];
        }
